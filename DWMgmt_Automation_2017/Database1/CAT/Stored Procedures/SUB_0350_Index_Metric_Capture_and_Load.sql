...
CREATE PROCEDURE [CAT].[SUB_0350_Index_Metric_Capture_and_Load]
...
, Schema_Name, Object_Name, Index_Name, Index_Type, Index_Type_Desc, Scan_Mode, Partition_Number, Alloc_Unit_Type_Desc, Index_Depth, Index_Level
...
FROM sys.dm_db_Index_physical_stats(db_ID(), NULL, NULL, NULL,'''+@ScanMode+''') as idxs
...
WHERE ('''+@NamePart+''' = ''ALL'' 
OR CHARINDEX('''+@NamePart+''', db_Name()+''.''+scm.name+''.''+obj.name) > 0)
'
