...
CREATE PROCEDURE [CAT].[SUB_1350_Index_Maintenance_Executor]
...
AND dmp.REG_Task_Proc = 'SUB_1350_INDEX_MAINTENANCE_DRIVER'
JOIN (
...
ORDER BY CASE WHEN dmp.REG_Task_Name = 'REORGANIZE_ONLINE'
	AND map.REG_Index_Type IN ('PK') THEN 1
	WHEN dmp.REG_Task_Name = 'REORGANIZE_ONLINE'
	AND map.REG_Index_Type NOT IN ('PK') THEN 2
	WHEN dmp.REG_Task_Name = 'REBUILD_ONLINE' 
	AND map.REG_Index_Type IN ('PK') THEN 3
	WHEN dmp.REG_Task_Name = 'REBUILD_ONLINE' 
	AND map.REG_Index_Type NOT IN ('PK') THEN 4
	ELSE 5 END
