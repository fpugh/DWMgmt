...
DECLARE @SourceDBLocation NVARCHAR(256) = 'DWMgmt'
...
/* Description: This procecdure executes a system scan. It executes several sub-procedures which utilize dynamic SQL to configure queries that allow for execution
	...
	It will acquire linked server information during its initial scan. Subsequent scans will permit wider and wider scan ranges, depending on link depth. An SSIS
...
/* Catalog Object Collection Tables */

EXEC CAT.MP_003_CENTRAL_REGISTRY_PROCESS
@ExecuteStatus = @ExecuteStatus


/* Begin Executing the Tracking Processing
	- Requires a second trip through the server	as basic catalog registrations are required to be completed by MP_003_CENTRAL_REGISTRY_PROCESS
*/
...
BEGIN
	...
	OPEN TrackingResolver

	FETCH NEXT FROM TrackingResolver INTO
	@TargetServer
	...
	BEGIN
		...
		/* SUB_1354_Data_Profile_Capture is NOT run here
				It is not safe without parameters narrowly defining 
				its profile targets. */

			SET @SQL = '
			EXEC CAT.SUB_1350_Index_Metric_Capture_and_Load
			@TargetDBLocation = N'''+@TargetDBLocation+''',
			@SourceDBLocation = N'''+@SourceDBLocation+''',
			@ExecuteStatus = '+CAST(@ExecuteStatus AS nvarchar)+';

			EXEC CAT.SUB_1300_Object_Utilization_Capture_and_Load
			@TargetDBLocation = N'''+@TargetDBLocation+''',
			@SourceDBLocation = N'''+@SourceDBLocation+''',
			@ExecuteStatus = '+CAST(@ExecuteStatus AS nvarchar)+';

			EXEC CAT.SUB_1203_Database_File_Change_Capture_and_Load
			@TargetDBLocation = N'''+@TargetDBLocation+''',
			@SourceDBLocation = N'''+@SourceDBLocation+''',
			@ExecuteStatus = '+CAST(@ExecuteStatus AS nvarchar)+';
			'
	...
	CLOSE TrackingResolver
	DEALLOCATE TrackingResolver
