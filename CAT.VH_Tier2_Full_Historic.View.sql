USE [DWMgmt]
GO
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[CAT].[VH_Tier2_Full_Historic]'))
DROP VIEW [CAT].[VH_Tier2_Full_Historic]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[CAT].[VH_Tier2_Full_Historic]'))
EXEC dbo.sp_executesql @statement = N'

CREATE VIEW [CAT].[VH_Tier2_Full_Historic]
AS
