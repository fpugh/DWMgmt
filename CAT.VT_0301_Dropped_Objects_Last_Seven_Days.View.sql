USE [DWMgmt]
GO
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[CAT].[VT_0301_Dropped_Objects_Last_Seven_Days]'))
DROP VIEW [CAT].[VT_0301_Dropped_Objects_Last_Seven_Days]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[CAT].[VT_0301_Dropped_Objects_Last_Seven_Days]'))
EXEC dbo.sp_executesql @statement = N'

CREATE VIEW [CAT].[VT_0301_Dropped_Objects_Last_Seven_Days]
AS
SELECT sub.REG_Database_Name, sub.REG_Schema_Name
, sub.REG_Object_Name, sub.REG_Object_Type
, count(*) as Incarnations
, max(Object_Term_Date) as last_termed_on
...
FROM (
	...
	, lsb.LNK_Term_Date as Object_Term_Date
	...
	WHERE lsb.LNK_Term_Date between getdate() - 7 and getdate()
