USE [DWMgmt]
GO
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[CAT].[VH_0300_Full_Object_Map]'))
DROP VIEW [CAT].[VH_0300_Full_Object_Map]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[CAT].[VH_0300_Full_Object_Map]'))
EXEC dbo.sp_executesql @statement = N'

CREATE VIEW [CAT].[VH_0300_Full_Object_Map]
AS
SELECT DENSE_Rank() OVER(ORDER BY LNK_T2_ID, LNK_T3_ID, isnull(lat3.LNK_Rank,0), lat3.LNK_Post_Date) AS VID
, lat1.LNK_T2_ID, lat2.LNK_T3_ID, lat3.LNK_T4_ID, reg1.REG_0100_ID, reg2.REG_0200_ID, reg3.REG_0204_ID, reg4.REG_0300_ID, reg5.REG_0400_ID
, reg1.REG_Server_Name
, reg2.REG_Database_Name+''.''+reg3.REG_Schema_Name+''.''+reg4.REG_Object_Name as Fully_Qualified_Name
, reg3.REG_Schema_Name+''.''+reg4.REG_Object_Name as Schema_Bound_Name
, reg2.REG_Database_Name, reg3.REG_Schema_Name, reg4.REG_Object_Name, reg4.REG_Object_Type
...
ON rcp.REG_0401_ID = lnk2.LNK_FK_0401_ID
