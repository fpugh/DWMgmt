...
CREATE TABLE #TopValues (Database_Name NVARCHAR(65), Schema_Bound_Name NVARCHAR(256), Column_Name NVARCHAR(65), Column_Type NVARCHAR(65), Max_Data_Length INT, Column_Value NVARCHAR(MAX), ROWID INT, LNK_T4_ID INT, Typical_Values NVARCHAR(max))
...
IF NOT EXISTS (SELECT name FROM sys.indexes WHERE name = N'idx_nc_TRK_0354_K1_I2')
CREATE NONCLUSTERED INDEX [idx_nc_TRK_0354_K1_I2]
ON [TMP].[TRK_0354_Value_Hash] ([LNK_T4_ID])
...
, MiniMax (LNK_T4_ID, Low_Column_Value, Low_Type_Flag, High_Column_Value, High_Type_Flag)
	...
	FROM (
		SELECT Column_Value, LNK_T4_ID, Numeric_Type_Flag
		FROM (
