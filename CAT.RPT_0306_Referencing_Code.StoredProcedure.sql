USE [DWMgmt]
GO
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[CAT].[RPT_0306_Referencing_Code]') AND type in (N'P', N'PC'))
DROP PROCEDURE [CAT].[RPT_0306_Referencing_Code]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[CAT].[RPT_0306_Referencing_Code]') AND type in (N'P', N'PC'))
BEGIN
...
, DENSE_Rank() OVER(PARTITION BY dbr.REG_Database_Name, dbs.REG_Schema_Name, REG_Object_Name
	ORDER BY COUNT(DISTINCT lnk.LNK_ID) DESC) as Line_References
