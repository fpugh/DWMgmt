USE [DWMgmt]
GO
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[CAT].[RPT_0303_Index_Coverage]') AND type in (N'P', N'PC'))
DROP PROCEDURE [CAT].[RPT_0303_Index_Coverage]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[CAT].[RPT_0303_Index_Coverage]') AND type in (N'P', N'PC'))
BEGIN
...
ON V200.REG_Database_Name = V343.REG_Database_Name
AND V200.REG_Schema_Name = V343.REG_Schema_Name
AND V200.REG_Object_Name = V343.REG_Object_Name
