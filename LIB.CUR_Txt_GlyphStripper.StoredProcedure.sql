USE [DWMgmt]
GO
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[LIB].[CUR_Txt_GlyphStripper]') AND type in (N'P', N'PC'))
DROP PROCEDURE [LIB].[CUR_Txt_GlyphStripper]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[LIB].[CUR_Txt_GlyphStripper]') AND type in (N'P', N'PC'))
BEGIN
